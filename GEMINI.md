...
### 2.8.1 Required Field Asterisk (`*`) Standard
* Every mandatory input field **MUST** render an unambiguous red asterisk `*` beside its label:
  `<span className="text-red-600 dark:text-red-400 ml-1" aria-hidden="true">*</span>`.
...
* When a field is required, never display `(Optional)` text beside the label.
* Non-mandatory fields should explicitly display `(Optional)` in subtle muted text (`text-gray-400 dark:text-gray-500 font-normal ml-1 text-xs`).

### 2.8.2 Always Enforce Strict Client-Side Validation (Zod + React Hook Form)
* **Pre-Submission Validation**: Every form submission must strictly validate all input fields using `Zod` schemas and `zodResolver` before any network request is initiated.
* **Standard Field Validation Rules**:
  - **Full Name**: Non-empty, trimmed, minimum 2 characters, maximum 100 characters. Error: `"Enter full name (at least 2 characters)"`.
  - **Email Address**: Valid email format matching standard RFC syntax. Error: `"Enter a valid email address (e.g. name@example.com)"`.
  - **Mobile / Phone Number**: Required numeric/standard phone format with 7 to 20 digits (`/^[0-9+()\- ]{7,20}$/`). Error: `"Enter a valid contact number (7 to 20 digits)"`.
  - **Password**: Minimum 6 characters, maximum 128 characters. Error: `"Password must be at least 6 characters"`.
  - **Status & Selection Enums**: Must validate against allowed enum options with clear feedback (e.g. `"Select an account status"`).
* **Instant Inline Visual Feedback**:
  - Errors must render immediately beneath the relevant input: `text-xs font-medium text-red-600 dark:text-red-400 mt-1`.
  - The input border must clearly shift to red: `border-red-500 focus:border-red-500 focus:ring-red-500/20`.
  - Form validation mode must trigger on submit and re-validate on change/blur (`mode: 'onTouched'` or `mode: 'onChange'`).

### 2.8.3 Always Enforce Strict Server-Side Validation (Pydantic + FastAPI)
* **Payload Validation**: Every endpoint receiving user input must define strict Pydantic request models with `Field(..., min_length=..., max_length=..., pattern=...)` and `EmailStr`.
* **Database & Uniqueness Constraints**:
  - Unique fields (email, mobile number, etc.) must be checked against the database before insertion or modification.
  - Return `HTTP 409 Conflict` or `HTTP 422 Unprocessable Entity` with clear field identification in `detail` when duplicates occur.
* **No Unhandled Server Crashes**: Database errors (e.g. `IntegrityError`) must be caught and returned as clean HTTP error responses, never raw 500 stack traces.

### 2.8.4 Proper Server-to-Field Error Mapping on Frontend
* **Programmatic Field Errors**: When backend APIs return validation errors (HTTP 422 with field paths or HTTP 409 conflict messages):
  - The frontend MUST parse the error and map it directly to the offending field using `setError(fieldName, { type: 'server', message })` from `react-hook-form`.
  - For example, if email already exists, set error on `email`: `"An account with this email already exists"`.
  - If mobile number already exists, set error on `mobile_number`: `"Phone number is already registered to another account"`.
  - The input border must turn red and the message must appear immediately below the input.
* **Global Error Banner**:
  - Top-level dismissible `<Alert variant="danger">` banners are reserved strictly for unexpected system errors (HTTP 500, network disconnects, unauthorized access) or non-field-specific errors.
  - When field-specific errors are present, they must be displayed under their respective inputs so the user instantly knows where to correct their entry.
