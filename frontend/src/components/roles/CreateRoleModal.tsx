...
import { z } from 'zod'
import { ShieldPlus, CheckSquare, Square, Search, X } from 'lucide-react'
...
interface CreateRoleModalProps {
  isOpen: boolean
  onClose: () => void
  onRoleCreated: (role: RoleItem) => void
}
...
export const CreateRoleModal: React.FC<CreateRoleModalProps> = ({
  ...
  useEffect(() => {
    ...
    const fetchPermissions = async () => {
      ...
      try {
        const response = await http.get<PermissionItem[]>(API_ENDPOINTS.PERMISSIONS.ALL)
        if (isMounted) {
          setAllPermissions(Array.isArray(response) ? response : [])
        }
      } catch {
        // Fallback: empty list
      } finally {
  ...
  const onSubmit = async (values: CreateRoleFormValues) => {
    ...
    try {
      ...
      const response = await http.post<RoleItem>(API_ENDPOINTS.ROLES.CREATE, payload)
      ...
      setServerError(
        (typeof apiErr.data?.detail === 'string' ? apiErr.data.detail : null) ||
          apiErr.data?.message ||
          'Failed to create role. Please verify your inputs and try again.'
      )
    }
  }
  ...
  return (
    <Dialog
      ...
      description="Define role name and select permission matrix"
      icon={<ShieldPlus className="w-5 h-5 text-blue-600 dark:text-blue-400" />}
      maxWidth="2xl"
    >
        ...
        <div className="border border-gray-200 dark:border-gray-800 rounded-xl p-3.5 bg-gray-50/70 dark:bg-gray-900/50 space-y-3">
          ...
          <div className="max-h-72 overflow-y-auto pr-1">
            {isLoadingPermissions ? (
              <div className="py-8 text-center text-sm text-gray-500 dark:text-gray-400 animate-pulse">
